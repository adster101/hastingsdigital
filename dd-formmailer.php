...
$script_path = 'http://hastings.digital/dd-formmailer.php';
...
$recipients = 'adam@hastings.digital';
...
$email_subject = 'Hastings Digital - Enquiry';
...
function ddfm_injection_chars($s) {
	...
	return (preg_replace("\r", $s) || preg_replace("\n", $s) || preg_replace("%0a", $s) || preg_replace("%0d", $s)) ? TRUE : FALSE;
}
		...
		if ($errors) {
			...
			if ($sndmsg == TRUE) {
				...
				if (ddfm_send_mail($recipients, $sender_name, $sender_email, $email_subject, $mail_message, $attach_save, $attach_path, $attached_files)) {
					...
					$o .= '<META HTTP-EQUIV="REFRESH" CONTENT="0;URL=http://hastings.digital/thanks/">';
